...
<div class="page-header mb-4 d-flex justify-content-between align-items-center">
    <div>
        <h3 class="h4 text-dark font-weight-bold">Gestión de Usuarios</h3>
    ...
    <div class="row align-items-center">
        <div class="col-md-5">
            ...
            <div class="input-group input-group-lg shadow-sm" style="border-radius: 8px; overflow: hidden;">
                ...
                <input type="text" id="searchInput" class="form-control border-0"
                    placeholder="Escribe nombre, usuario, rol o email..." style="font-size: 0.95rem; box-shadow: none;">
            </div>
        </div>
        <div class="col-md-7 d-flex justify-content-end mt-3 mt-md-0">
            <span
                ...
                <i class="fas fa-users me-1"></i> Total: <span id="userCount" class="fw-bold">
                    <?php echo $result_users->num_rows; ?>
                </span>
            </span>
        </div>
    </div>
...
<div class="table-responsive">
    <table class="table table-hover w-100 align-middle" id="usersTable">
        <thead class="table-light text-muted small text-uppercase"
            style="border-top: 2px solid #e3e6f0; border-bottom: 2px solid #e3e6f0;">
            <tr>
        ...
        <tbody>
            ...
            while ($row = $result_users->fetch_assoc()):
                ...
                <tr class="user-row" data-search="<?php echo htmlspecialchars($searchData); ?>"
                    ...
                    <td class="text-muted fw-bold small ps-3">
                        #
                        <?php echo str_pad($row['id_usuario'], 3, '0', STR_PAD_LEFT); ?>
                    </td>
                    <td>
                    ...
                    <td class="text-center">
                        <?php if (strtolower($row['role']) == 'administrador'): ?>
                            <span
                                class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 rounded-pill px-3 py-2 fw-semibold shadow-sm">
                                <i class="fas fa-shield-alt me-1"></i> Admin
                            </span>
                        <?php elseif (strtolower($row['role']) == 'pastor'): ?>
                            <span
                                class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 rounded-pill px-3 py-2 fw-semibold shadow-sm">
                                <i class="fas fa-bible me-1"></i> Pastor
                            </span>
                        <?php else: ?>
                            <span
                                class="badge bg-info bg-opacity-10 text-info border border-info border-opacity-25 rounded-pill px-3 py-2 fw-semibold shadow-sm">
                                <i class="fas fa-user-tie me-1"></i> Secretario
                            </span>
                        <?php endif; ?>
                    </td>
                    <td class="text-center">
                        <?php if (!empty($row['nombre_grupo'])): ?>
                            <span class="badge"
                                style="background:#eff6ff;color:#1e40af;border-radius:6px;padding:.4em .8em;font-size:0.8rem;">
                                <i class="fas fa-layer-group me-1" style="font-size:0.7rem;"></i>
                                <?php echo htmlspecialchars($row['nombre_grupo']); ?>
                            </span>
                        <?php else: ?>
                    ...
                    <td class="text-center">
                        <?php if (strtolower($row['status']) == 'activo'): ?>
                            <span
                                class="badge bg-success bg-opacity-10 text-success rounded-pill px-3 py-2 fw-semibold shadow-sm">
                                <i class="fas fa-circle me-1" style="font-size: 0.6rem;"></i> Activo
                            </span>
                        <?php else: ?>
                            <span
                                class="badge bg-secondary bg-opacity-10 text-secondary rounded-pill px-3 py-2 fw-semibold shadow-sm">
                    ...
                    <td class="text-center pe-3">
                        <div class="d-flex justify-content-center align-items-center gap-2">
                            <!-- Botón Editar -->
                            <button class="btn btn-sm text-primary btn-edit-user hover-elevate" title="Editar Usuario"
                                style="background: rgba(78, 115, 223, 0.1); border-radius: 8px; border: 1px solid rgba(78, 115, 223, 0.2); width: 34px; height: 34px; transition: all 0.2s;"
                                ...
                                data-grupo="<?php echo (int) ($row['grupo_id'] ?? 0); ?>">
                                <i class="fas fa-edit"></i>
                            </button>

                            <!-- Formulario Automático de Cambio de Estado -->
                            <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>"
                                class="m-0 p-0 d-inline-block form-toggle-status">
                                <input type="hidden" name="action" value="toggle_status">
                                <input type="hidden" name="id_usuario" value="<?php echo $row['id_usuario']; ?>">
                                <input type="hidden" name="current_status" value="<?php echo $row['status']; ?>">

                                <?php if (strtolower($row['status']) == 'activo'): ?>
                                    <button type="button" class="btn btn-sm text-danger btn-confirm-status hover-elevate"
                                        title="Desactivar Usuario"
                                        style="background: rgba(231, 74, 59, 0.1); border-radius: 8px; border: 1px solid rgba(231, 74, 59, 0.2); width: 34px; height: 34px; transition: all 0.2s;">
                                        <i class="fas fa-power-off"></i>
                                    </button>
                                <?php else: ?>
                                    <button type="button" class="btn btn-sm text-success btn-confirm-status hover-elevate"
                                        title="Activar Usuario"
                                        style="background: rgba(28, 200, 138, 0.1); border-radius: 8px; border: 1px solid rgba(28, 200, 138, 0.2); width: 34px; height: 34px; transition: all 0.2s;">
                            ...
                            <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>"
                                class="m-0 p-0 d-inline-block form-delete-user">
                                <input type="hidden" name="action" value="delete_user">
                                <input type="hidden" name="id_usuario" value="<?php echo $row['id_usuario']; ?>">
                                <button type="button" class="btn btn-sm text-danger btn-delete-user hover-elevate"
                            ...
                            <select class="form-select border-start-0 ps-0" id="grupo_id" name="grupo_id"
                                ...
                                <?php foreach ($grupos_list as $grp): ?>
                                    <option value="<?php echo $grp['id']; ?>">
                                        <?php echo htmlspecialchars($grp['nombre']); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
...
<!-- Estilos para animaciones de tabla -->
<style>
    .hover-elevate:hover {
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const myModalEdit = modalEditEl ? new bootstrap.Modal(modalEditEl) : null;
        ...
        editButtons.forEach(btn => {
            btn.addEventListener("click", function () {
                ...
                // Setear Grupo
                const grupoSelect = document.getElementById('edit_grupo_id');
                if (grupoSelect) grupoSelect.value = this.getAttribute('data-grupo') || '';
