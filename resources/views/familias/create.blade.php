...
@section('header_title', 'Registro de Familia')
@section('header_subtitle', 'Organización y consolidación del núcleo de la iglesia')
@section('header_icon')
...
<div class="py-8 px-4 max-w-5xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-8 flex-wrap gap-4 border-b border-slate-200 dark:border-slate-800/80 pb-6">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mb-0 font-medium">Defina el nombre del núcleo familiar para agrupar a sus miembros</p>
        </div>
    ...
    <div class="bento-card bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-3xl p-8 shadow-xl relative overflow-hidden group">
        ...
        <form action="{{ route('familias.store') }}" method="POST" class="m-0 space-y-10">
            @csrf
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="nombre" value="{{ old('nombre') }}" placeholder="Ej: Familia Pérez García" required
                        ...
                        <div class="mt-1.5 text-[11px] text-slate-400 dark:text-slate-500 font-medium">Sugerencia: Use los apellidos de los padres o el nombre del titular.</div>
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="notas" rows="2" placeholder="Información adicional relevante sobre la familia..."
                                  class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800 px-4 py-3.5 text-slate-900 dark:text-white text-xs font-medium focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm resize-none">{{ old('notas') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="pt-8 border-t border-slate-200 dark:border-slate-800 flex items-center justify-end gap-3">
                <a href="{{ route('familias.index') }}" class="px-6 py-3.5 rounded-2xl text-xs font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all no-underline cursor-pointer">Cancelar</a>
                <button type="submit" class="btn-bento-primary px-8 py-3.5 rounded-2xl font-bold text-xs flex items-center justify-center gap-2.5 transition-all cursor-pointer">
                    ...
                    <span>Crear Familia Oficial</span>
                </button>
            </div>
        </form>
    </div>
