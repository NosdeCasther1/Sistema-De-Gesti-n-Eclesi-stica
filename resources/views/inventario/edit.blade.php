@extends('layouts.app')
@section('title', 'Editar Artículo | ' . ($globalConfig->nombre_iglesia ?? 'AD REY DE REYES'))
@section('header_icon') <i class="fas fa-edit text-xl"></i> @endsection
@section('header_title', 'Editar Artículo')
@section('header_subtitle', 'Modificar inventario')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="card-module bg-white dark:bg-slate-900 shadow-sm border border-gray-100 dark:border-slate-800 rounded-2xl overflow-hidden">
        <form action="{{ route('inventario.update', $inventario) }}" method="POST" class="p-6 md:p-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="estado" class="w-full bg-gray-50 dark:bg-slate-800 border-gray-200 dark:border-slate-700 rounded-xl px-4 py-3 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all shadow-sm @error('estado') border-red-500 @enderror" required>
                        ...
                        <option value="Malo" {{ old('estado', $inventario->estado) == 'Malo' ? 'selected' : '' }}>Malo</option>
                    </select>
