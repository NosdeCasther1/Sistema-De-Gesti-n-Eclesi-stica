                                        ...
                                        <div class="row mb-4 align-items-center">
                                            ...
                                            <div class="col">
                                                ...
                                                <input type="file" class="form-control" name="logo" id="logo_input"
                                                    accept=".png,.jpg,.jpeg,.webp" onchange="validarYPrevisualizar(this)">
                                                <div class="mt-1">
                                                    <small class="text-muted d-block"><i class="fas fa-info-circle me-1"></i>Formatos: <strong>PNG, JPG, WEBP</strong>.</small>
                                                    <small class="text-muted d-block"><i class="fas fa-weight-hanging me-1"></i>Tamaño máximo: <strong>2MB</strong>.</small>
                                                </div>
                                            </div>
                                        </div>
...
<script>
    function validarYPrevisualizar(input) {
        if (input.files && input.files[0]) {
            ...
            if (file.size > maxSize) {
                Swal.fire({
                    ...
                    text: 'El logotipo no debe superar los 2MB. Tu archivo pesa ' + (file.size / (1024 * 1024)).toFixed(2) + 'MB.',
    ...
    document.getElementById('formConfiguracion').addEventListener('submit', function (e) {
        ...
        fetch('/ProyectoIglesia/configuracion/guardar', {
            method: 'POST',
            body: formData
        })
    ...
    function descargarBackup() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = '/ProyectoIglesia/_/generar_backup';
            }
        });
    }
    ...
    document.getElementById('formRestore').addEventListener('submit', function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch('/ProyectoIglesia/_/procesar_restore', {
                    method: 'POST',
                    body: formData
                })
                    .then(response => response.json())
