 ...
 */
function analyzeFile(filePath: string, relativePath: string): Vulnerability[] {
      ...
      case "check_rbac_middleware": {
        ...
        if (!controllerPath || !fs.existsSync(controllerPath)) {
          throw new McpError(
            ...
            `Controller class could not be resolved or found. Tried matching: '${controller}'`
          );
        }
        ...
        const hasRoleMiddleware = registeredMiddleware.some(mw => 
          mw.toLowerCase().includes("role") || 
